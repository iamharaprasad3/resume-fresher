...
st.set_page_config(page_title="Addverb Resume Shortlister - Fresher", page_icon="https://addverb.com/wp-content/uploads/2023/12/cropped-MicrosoftTeams-image-7.png", layout="centered")
keywords_para =  st.text_area("Please input keywords separated by a comma!")
job_responsibilites = st.text_area("Please insert the job responsibilties")
...
def runningmain(text_content, file_name):
    total_score = 0
    dicc ={}
    ...
    # similarity score
    # job_responsibilites = extract_content_between_keywords(text, 'Job Responsibilities', '1')
    similarity_score = get_similarity_score(job_responsibilites, text_content)
...
if(jd_done):
    ...
    if(uploaded_resumes is not None):
        for uploaded_resume in uploaded_resumes:
            ...
            runningmain(text_content, uploaded_resume.name)
...
df = pd.DataFrame(data, columns=["File Name","Keyword Matching Percentage", "Similarity Score", "Result", "Total Score"])
